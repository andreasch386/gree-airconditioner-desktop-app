...
public class GreeHvacControllerApp extends Application {
  ...
  private javafx.scene.layout.HBox createHeaderWithControls() {
    ...
    currentTempLabel.setStyle(
        "-fx-text-fill: #FFE082; -fx-font-size: 18px; -fx-font-weight: bold;");
  ...
  private javafx.scene.layout.HBox createHeaderControls() {
    ...
    powerToggle.setStyle(
        "-fx-background-color: #FF5722; -fx-text-fill: white; -fx-font-weight: bold; -fx-padding: 6px 12px; -fx-min-width: 60px;");
  ...
  private VBox createMergedDeviceSection() {
    ...
    javafx.scene.control.Label sectionTitle =
        new javafx.scene.control.Label("Device Selection & Connection");
    ...
    deviceComboBox.setOnAction(
        e -> {
          String selected = deviceComboBox.getValue();
          if (selected != null && !selected.isEmpty()) {
            com.gree.hvac.dto.DeviceInfo newSelectedDevice =
            ...
            if (newSelectedDevice != null) {
              ...
              if (currentClient != null
                  ...
                  && selectedDevice != null
                  && !newSelectedDevice.equals(selectedDevice)) {
  ...
  private void startBackgroundDiscovery() {
    ...
    discoveryTask =
        discoveryExecutor.scheduleAtFixedRate(
            this::performDiscovery, 30, 30, java.util.concurrent.TimeUnit.SECONDS);
  }
  ...
  private void updateDeviceComboBox() {
    ...
    if (currentSelection != null && deviceComboBox.getItems().contains(currentSelection)) {
      ...
      String selectedDisplayName =
          selectedDevice.getName() + " (" + selectedDevice.getIpAddress() + ")";
  ...
  private void startStatusPolling() {
    ...
    statusPollingTask =
        statusPollingExecutor.scheduleAtFixedRate(
            this::updateDeviceStatus, 1, 3, java.util.concurrent.TimeUnit.SECONDS);
  }
