...
public class MainController {
    ...
    @FXML
    private TableColumn<DeviceInfo, String> nameColumn;
    
    @FXML
    private TableColumn<DeviceInfo, String> ipColumn;
    
    @FXML
    private TableColumn<DeviceInfo, String> modelColumn;
    
    @FXML
    private TableColumn<DeviceInfo, String> statusColumn;
    ...
    @FXML
    private ToggleButton turboToggle;
    
    @FXML
    private ToggleButton quietToggle;
    ...
    @FXML
    private ProgressIndicator discoveryProgress;
    ...
    private void setupDeviceTable() {
        ...
        deviceTable.getSelectionModel().selectedItemProperty().addListener(
            (observable, oldValue, newValue) -> onDeviceSelectionChanged(newValue)
        );
    }
    ...
    private void setupControlPanel() {
        ...
        // Power toggle setup
        powerToggle.selectedProperty().addListener((observable, oldValue, newValue) -> {
    ...
    private void setupComboBoxes() {
        ...
        fanSpeedComboBox.setOnAction(event -> {
            if (currentClient != null && currentClient.isConnected()) {
    ...
    private void onDiscoverDevices() {
        ...
        discoveryFuture.thenAccept(devices -> {
            Platform.runLater(() -> {
                ...
                if (devices.isEmpty()) {
                    showAlert("No devices found", "No GREE HVAC devices were discovered on the network.");
                } else {
            ...
            Platform.runLater(() -> {
                setDiscoveryProgress(false);
                discoverButton.setDisable(false);
                showAlert("Discovery Error", "Failed to discover devices: " + throwable.getMessage());
            });
            return null;
        });
    }
    ...
    private void onDeviceSelectionChanged(DeviceInfo device) {
        ...
        if (device != null) {
            deviceInfoLabel.setText(String.format("Selected: %s (%s)", device.getName(), device.getIpAddress()));
            connectButton.setDisable(false);
        } else {
    ...
    private void updateDevicePower(boolean power) {
        ...
        currentClient.control(control)
            ...
            .exceptionally(throwable -> {
                ...
                Platform.runLater(() -> showAlert("Control Error", "Failed to update power: " + throwable.getMessage()));
                return null;
            });
    }
    ...
    private void updateDeviceTemperature(double temperature) {
        ...
        currentClient.control(control)
            ...
            .exceptionally(throwable -> {
                ...
                Platform.runLater(() -> showAlert("Control Error", "Failed to update temperature: " + throwable.getMessage()));
                return null;
            });
    }
    ...
    private void updateDeviceMode(String mode) {
        ...
        currentClient.control(control)
            ...
            .exceptionally(throwable -> {
                ...
                Platform.runLater(() -> showAlert("Control Error", "Failed to update mode: " + throwable.getMessage()));
                return null;
            });
    }
    ...
    private void updateDeviceFanSpeed(String fanSpeed) {
        ...
        currentClient.control(control)
            ...
            .exceptionally(throwable -> {
                ...
                Platform.runLater(() -> showAlert("Control Error", "Failed to update fan speed: " + throwable.getMessage()));
                return null;
            });
    }
